#!/usr/bin/env bash
# Package, notarize, and staple a Developer ID–signed Storefront DMG.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
ENTITLEMENTS="$ROOT/Storefront/Resources/Storefront.entitlements"
...
Verifies (or applies) Developer ID signing, builds dist/Storefront.dmg,
...
Prerequisites:
  ...
  3. Config/Signing.local.xcconfig with DEVELOPMENT_TEAM (from example)
  4. Notary credentials — one of:
       - Keychain profile:  xcrun notarytool store-credentials storefront-notary
         then: NOTARY_PROFILE=storefront-notary (default)
...
Environment:
  ...
  NOTARY_PROFILE     notarytool keychain profile (default: storefront-notary)
...
APP_PATH="${1:-}"
if [[ -z "$APP_PATH" ]]; then
  die "path to Storefront.app is required"$'\n\n'"$(usage)"
fi
...
if [[ -n "${NOTARY_KEY_PATH:-}" && -n "${NOTARY_KEY_ID:-}" && -n "${NOTARY_ISSUER_ID:-}" ]]; then
  ...
  PROFILE="${NOTARY_PROFILE:-storefront-notary}"
